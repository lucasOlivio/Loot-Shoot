...
protected:
    void SetUp() override {
        m_pScene = std::shared_ptr<Scene>(new Scene());
...
TEST_F(SceneSerializerTest, DeserializeTest) {
    ...
    EXPECT_EQ(m_pScene->GetEntityCount(), 3); // 1 more for the singleton components
    ...
    EXPECT_TRUE(m_pScene->HasComponent(1, tag));
    EXPECT_TRUE(m_pScene->HasComponent(1, transform));
    EXPECT_TRUE(m_pScene->HasComponent(1, movement));

    EXPECT_TRUE(m_pScene->HasComponent(2, tag));
    EXPECT_TRUE(m_pScene->HasComponent(2, transform));
    EXPECT_FALSE(m_pScene->HasComponent(2, movement));
}
