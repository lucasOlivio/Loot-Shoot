...
protected:
    ...
    std::shared_ptr<iEventBus<eWindowEvents, WindowCloseEvent>> eventBus;
};
...
TEST_F(EventBusTest, AccessUnsetEventBus)
{
    ...
    std::shared_ptr<iEventBus<eWindowEvents, WindowCloseEvent>> nullEventBus = EventBusLocator<eWindowEvents, WindowCloseEvent>::Get();
