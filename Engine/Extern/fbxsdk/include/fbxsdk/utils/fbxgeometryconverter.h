...
public:
	...
	//@{
		...
		* currently only supports node attribute of type eMesh, ePatch, eNurbs or eNurbsSurface. */
		bool Triangulate(Fbxstd::shared_ptr<Scene> pScene, bool pReplace, bool pLegacy=false);
    ...
    */
	...
	//@{
		...
		* \remark The function will still iterate through all meshes regardless if one fails to split, but will return false in that case. */
		bool SplitMeshesPerMaterial(Fbxstd::shared_ptr<Scene> pScene, bool pReplace);
	...
	* \return \c true if any nodes were re-centered, otherwise \c false. */
	bool RecenterSceneToWorldCenter(Fbxstd::shared_ptr<Scene> pScene, FbxDouble pThreshold);
	...
	*/
	FbxNode* MergeMeshes(FbxArray<FbxNode*>& pMeshNodes, const char* pNodeName, Fbxstd::shared_ptr<Scene> pScene);
	...
	*/
	void RemoveBadPolygonsFromMeshes(Fbxstd::shared_ptr<Scene> pScene, FbxArray<FbxNode*>* pAffectedNodes = NULL);
