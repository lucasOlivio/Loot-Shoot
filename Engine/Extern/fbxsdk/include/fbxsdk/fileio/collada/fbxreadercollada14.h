...
private:
    ...
    */
    ...
    /** Import a COLLADA rotation element.
      ...
      * \param rotationVector Vector4 value to return the rotation vector.
      * \return Return the rotation axis index.
      */
    int ImportRotationElement(xmlNode* pXmlNode, FbxVector4& rotationVector);

    /** Extrapolate rotation order by the given int list.
      * \param pNode The node whose rotation order is updated.
      * \param rotationOrder The int list representing the rotation order.
      */
    void SetRotationOrder(FbxNode * pNode, const FbxArray<int> & rotationOrder);
    ...
    */
    ...
    /** Import the transform animation of a given node.
      ...
      * \animationChannelID The ID of animation element.
      * \return Return \c true on success and \c false otherwise.
      */
    bool ImportMatrixAnimation(FbxNode * pNode, const FbxString & animationChannelID);

    /** Import the animation of a given attribute.
      * \param pProperty The property to be animated.
      * \param animationChannelID The ID of animation element.
      * \param pChannelName The channel name if the property has multiple channels.
      * \return Return \c true on success and \c false otherwise.
      */
    bool ImportPropertyAnimation(FbxProperty & pProperty, const FbxString & animationChannelID, const char * pChannelName = NULL);

    /** Get the animation layer to which the specific animation curve belongs.
      * \param animationID The ID of the animation curve.
      * \return The animation layer.
      */
    FbxAnimLayer * GetAnimLayer(const FbxString & animationID);
    ...
    Fbxstd::shared_ptr<Scene>		mScene;
